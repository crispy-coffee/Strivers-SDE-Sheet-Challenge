...
public:
    ...
    void insert(int num) {
        Node* prev = root;
        for (int i = 31; i >= 0; i--) { // inserting a 32-bit integer starting from the most significant bit
            int bit = (num >> i) & 1;
            Node* child;
...
vector<int> maxXorQueries(vector<int>& arr, vector<vector<int>>& queries) {
    ...
    for (int i = 0; i < offlineQuery.size(); i++) {
        ...
        while (idx < arr.size() && arr[idx] <= ai) {
            t.insert(arr[idx]);
            idx++; // we don't initialize it again to 0 by putting idx in the for loop as the array is sorted hence all the previous elements 
